export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(url.toString(), {
      headers: {
        'Referer': 'https://jibunmoa.vercel.app/',
        'Origin': 'https://jibunmoa.vercel.app',
        'User-Agent': 'Mozilla/5.0 (compatible; jibunmoa/1.0)',
      }
    });
    const text = await response.text();
    // HTML 에러페이지 반환 시 처리
    if (text.trim().startsWith('<')) {
      return res.status(502).json({ error: 'vworld returned HTML', raw: text.substring(0, 200) });
    }
